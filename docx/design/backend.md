...
#### 数据库设计
- 使用MySQL数据库，包含表：user（用户）、pokemon（宝可梦）、skill（技能）、backpack（背包）、pokemon_stats（统计数据）等。
- 支持外键关联和索引优化。
...
- **打包**：mvn clean package 生成JAR文件，支持Docker部署。
- **测试**：JUnit单元测试，确保代码质量。
